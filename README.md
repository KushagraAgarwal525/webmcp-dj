...
Agent craft: `get_dj_playbook` → `get_crate_health` → `plan_set_arc` → `get_mix_points` → `apply_transition_recipe` → `preview_join` → `verify_set` → `set_propose`. Recipes compile to bass swaps, double-drops, loop-rolls, backspins — not linear washes.
...
- Set: peak-first arc planner, listen-score joins, power-block trims, recipe compiler
...
4. Set → **Propose** (or Agent → `set_propose` with `{"duration":"30 minutes","energy_arc":"warm-up to peak"}`)
5. Accept proposal → Play
